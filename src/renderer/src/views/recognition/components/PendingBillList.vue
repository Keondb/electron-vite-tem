...
import { getListsApi, getListsAfterIdApi } from '@/api/recognition'
import { Howl } from 'howler'

// 正确引用 mp3 文件
const orderSoundPath = new URL('@/assets/audio/y2085.mp3', import.meta.url).href

// 创建提示音
const orderNotificationSound = new Howl({
  src: [orderSoundPath], // 放在 public/sounds 目录下
  volume: 1.0
})
...
// 轮询相关
const pollingInterval = ref(30000) // 轮询间隔，默认30秒
...
const loadMoreData = async () => {
  if (!lastTimestampOrId.value || loading.value) {
    return
  }

  try {
    // 调用API获取指定ID之后的数据
    const response = await getListsAfterIdApi({
      id: lastTimestampOrId.value,
      is_finish: 0 // 待处理状态
    })
    ...
    if (response && response.code === 200 && response.data) {
      ...
      if (backendData.length > 0) {
        ...
        // 将新数据转换为前端需要的格式并追加到现有数据中
        const newData = backendData.map((item) => ({
          time: item.create_time,
          admin: item.admin_name,
          company: item.company_name,
          is_flaw: item.is_flaw,
          billNumber: `BILL${item.id.toString().padStart(3, '0')}`,
          amount: parseFloat(item.total_amount),
          status: item.is_finish === 0 ? '待处理' : '已完成',
          id: item.id,
          flawCount: item.flawCount,
        ...
        // 更新总数据量
        totalDataCount.value = response.data.total || 0
        orderNotificationSound.play() // 播放提示音
      }
    } else {
      console.error('获取更多数据失败')
    }
  } catch (error) {
    console.error('轮询加载数据失败', error)
  }
}
      ...
      >
        ...
        <div class="company-info">
          <div class="company-stats">
            <span class="company-stat-item" style="cursor: pointer" @click="showModal(row)">
        ...
        <div class="item-footer">
          <div class="action-wrapper">
            ...
            <!-- <span
              v-if="row.is_flaw === 2"
              class="flaw-warning"
              style="margin-left: 5px; margin-right: 15px; cursor: pointer;background-color: red; padding: 6px;color: #fff;"
              @click.stop="emit('showFlawDetails', row)"
              >瑕疵 [<span style="color: #ff0000; font-size: 14px; font-weight: bold"> 1 </span>]
              张</span
            > -->
